...
/** 与 {@link AiService} 一致：Nest 注入的 LangChain tool 实例类型 */
...
export class JobAgentService {
  ...
  constructor(
    ...
    @Inject('SEND_MAIL_TOOL') private readonly sendMailTool: InjectableTool,
    @Inject('WEB_SEARCH_TOOL') private readonly webSearchTool: InjectableTool,
    @Inject('DB_USERS_CRUD_TOOL')
    private readonly dbUsersCrudTool: InjectableTool,
    @Inject('TIME_NOW_TOOL') private readonly timeNowTool: InjectableTool,
  ) {
