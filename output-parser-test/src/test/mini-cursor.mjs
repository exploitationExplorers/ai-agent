...
- 创建 Vite 项目时，优先使用 npx，避免 npm create 在 Windows 下吞掉 --template：
  npx create-vite@latest react-todo-app --template react-ts --no-interactive
- 不要使用 "n\\n"、管道符 "|" 或其他模拟按回车的写法
...
1. 创建项目：npx create-vite@latest react-todo-app --template react-ts --no-interactive
