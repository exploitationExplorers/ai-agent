          ...
          async ({
            ...
            switch (action) {
              case 'list': {
                ...
                const lines = jobs
                  .map((j: Job & { running: boolean }) => {
                    const atStr =
                      j.at instanceof Date ? j.at.toISOString() : (j.at ?? '');
                    return `id=${j.id} type=${j.type} enabled=${j.isEnabled} running=${j.running} cron=${j.cron ?? ''} everyMs=${j.everyMs ?? ''} at=${atStr} instruction=${j.instruction ?? ''}`;
                  })
                  .join('\n');
              ...
              case 'add': {
                ...
                if (type === 'at') {
                  ...
                  const atOut = created.at ? created.at.toISOString() : '';
                  return `已新增定时任务：id=${created.id} type=at at=${atOut} enabled=${created.isEnabled}`;
                }
