...
/** 供 Nest factory 使用，避免 `svc.tool` 在 providers 数组上下文中被推断为 `any` 而触发 no-unsafe-return */
function langchainTool<S extends { readonly tool: StructuredToolInterface }>(
  svc: S,
...
@Module({
  ...
  providers: [
    ...
    },
    {
      ...
      useFactory: (svc: SendMailToolService) => langchainTool(svc),
      inject: [SendMailToolService],
    },
    {
      provide: 'WEB_SEARCH_TOOL',
      useFactory: (svc: WebSearchToolService) => langchainTool(svc),
      inject: [WebSearchToolService],
    },
    {
      provide: 'DB_USERS_CRUD_TOOL',
      useFactory: (svc: DbUsersCrudToolService) => langchainTool(svc),
      inject: [DbUsersCrudToolService],
    },
    {
      provide: 'TIME_NOW_TOOL',
      useFactory: (svc: TimeNowToolService) => langchainTool(svc),
      inject: [TimeNowToolService],
    },
    {
      provide: 'CRON_JOB_TOOL',
      useFactory: (svc: CronJobToolService) => langchainTool(svc),
